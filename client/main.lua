...
local function axe()
    ...
    TriggerEvent('atu_kaevandus:notify', 'Puuraidur', Config.Alerts["error_axe"], 'error')
end
...
RegisterNetEvent('esx-lumberjack:vehicle', function()
    ...
    if not IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
        ...
        TriggerEvent('atu_kaevandus:notify', 'Puuraidur', Config.Alerts["depo_blocked"], 'error')
    end
end)
...
RegisterNetEvent('esx-lumberjack:removevehicle', function()
    ...
    TriggerEvent('atu_kaevandus:notify', 'Puuraidur', Config.Alerts["depo_stored"], 'success')
end)
...
RegisterNetEvent('esx-lumberjack:bossmenu', function(data)
    --print(json.encode(data, {indent=true}))
    lib.registerContext({
        id = 'bossmenu_soiduk',
        title = Config.Alerts["vehicle_header"],
        menu = 'bossmenu_soiduk',
        options = {
            --{title = Config.Alerts["vehicle_text"]},
            {
                title = Config.Alerts["vehicle_header"],
                description = 'Puuraiduri sõiduk',
                arrow = true,
            ...
            },
            {
                ...
                description = 'Soeta omale kirves',
                arrow = true,
    ...
    lib.showContext('bossmenu_soiduk')
end)
...
RegisterNetEvent('esx-lumberjack:processormenu', function(data)
    --print(json.encode(data, {indent=true}))
    lib.registerContext({
        ...
        options = {
            --{title = Config.Alerts["vehicle_text"]},
            {
                ...
                description = 'Lase Igoril puitu töödelda',
                arrow = true,
...
RegisterNetEvent('esx-lumberjack:processor', function()
    ESX.TriggerServerCallback('esx-lumberjack:lumber', function(lumber)
      if lumber then
        ...
        if lib.progressBar({
            ...
            TriggerEvent('atu_kaevandus:notify', 'Puuraidur', Config.Alerts['cancel'], 'inform')
        end
        -- TriggerEvent("mythic_progbar:client:progress", {
        --     name = "lumber_trader",
        --     duration = LumberJob.ProcessingTime,
    ...
    --     end)
         ...
         TriggerEvent('atu_kaevandus:notify', 'Puuraidur', Config.Alerts['error_lumber'], 'error')
    end
    end)
  end)
